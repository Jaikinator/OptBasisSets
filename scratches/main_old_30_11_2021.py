...
class dft_system:
    ...
    def scf_dft_calc(self):
        ...
        mf =dft.RKS(self.mol).apply(scf.addons.remove_linear_dep_) #using kohn sham
        # mf = scf.RHF(self.mol) #using Hatree Fock
        mf.kernel()
...
def system_init(atomstruc, basis1, basis2, **kwargs):
    ...
    if type(atomstruc) is list:
        system = dft_system(basis1, atomstruc, scf = False, requires_grad = True, **kwargs)
        system_ref = dft_system(basis2, atomstruc,**kwargs)
    elif type(atomstruc) is str:
        system = system_ase(basis1, atomstruc, scf = False, requires_grad = True,**kwargs)
...
def projection(coeff : torch.Tensor, colap : torch.Tensor, num_gauss : torch.Tensor):
    ...
    """
    S_11, S_12, S_21, _ = _cross_selcet(colap, num_gauss)
...
if __name__ == "__main__":
    ...
    print("basis scf initial:", system_ase(basis, atomstruc).get_basis_scf)
