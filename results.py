...
#delete all plots in evaluation/plots folder
for file in os.listdir("evaluation/plots"):
    os.remove("evaluation/plots/" + file)
    print("removed: ", file)

#make a scatter plot using  'best rel. improvement %', 'mean rel. improvement %' and highlight the best results
for i in range(len(dfdf)):
    plt.scatter(dfdf.loc[i]["best rel. improvement %"], dfdf.loc[i]["mean rel. improvement %"], label=dfdf.loc[i]["molecule"])
    #plt.annotate(dfdf.loc[i]["molecule"], xy=(dfdf.loc[i]["best rel. improvement %"], dfdf.loc[i]["mean rel. improvement %"]))
...
def create_plots_old(df):
    # fig, ax = plt.subplots(1,1)
    # ax.scatter(df.index,abs(df['initial_energy']), label = 'initial_energy')
    # ax.scatter(df.index,abs(df["ref_energy"]), label ="ref_energy")
    # ax.scatter(df.index,abs(df['opt_energy']), label ='opt_energy')
    ...
    for mol in molec:
        ...
        yval_sto3g = df.loc[indx]['initial_energy']
        yval_sto3g_opt = df.loc[indx]["opt_energy"]
        yval_cc_pvtz = df.loc[indx]["ref_energy"]
